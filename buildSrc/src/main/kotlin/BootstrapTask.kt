    ...
    private fun getArtifacts(): Array<JsonBuilder> {
        ...
        project.configurations["runtimeClasspath"].resolvedConfiguration.resolvedArtifacts.forEach {
            ...
            if (it.file.name.contains("injected-client") ||
                ...
                path = "https://github.com/mithruir/open-osrs-hosting/raw/master/${type}/${it.file.name}"
            } else if (it.file.name.contains("injection-annotations")) {
                path = "https://github.com/mithruir/open-osrs-hosting/raw/master/" + group.replace(".", "/") + "/${name}/$version/${it.file.name}"
        ...
        artifacts.add(JsonBuilder(
                "name" to cjar.name,
                "path" to "https://github.com/mithruir/open-osrs-hosting/raw/master/${type}/${cjar.name}",
                "size" to cjar.length(),
